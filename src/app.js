...
app.post("/tag_id-user", async (req, res) => {
	try {
		...
		res.json({
			...
			lastTagCreatedAt: newBadgeBonding.toObject().updatedAt,
		});
	} catch (error) {
		console.log(error);
		res.status(500).json({
					...
					user_type: {
						$switch: {
							branches: [
								...
								{
									case: { $gt: ["$has_delivered_tags", 0] },
									then: "tags_with_delivered",
								},
							],
...
app.get("/delivered-status", async (req, res) => {
	try {
		let { delivered, skip, limit } = req.query;
		...
		const countAttendees = (await User.aggregate(aggregation)).length;
		...
		res.status(200).json({
			data: { attendees, count: countAttendees },
